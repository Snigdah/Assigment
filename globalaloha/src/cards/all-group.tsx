import React from 'react';
import CardImage from './card-img';
import { Spin } from 'antd';

const AllGroup = (props) => {
  const { data } = props;
  console.log(' asd as', data);
  return (
    <>
      {!data.length ? (
        <div style={{ position: 'fixed', top: '30%', left: '48%' }}>
          <Spin size="large" />
        </div>
      ) : (
        <div className="grid grid-cols-3 gap-4" style={{ height: '100vh' }}>
          {data.map((d, i) => (
            <div className="flex h-screen items-center justify-center px-4">
              <div className="max-w-sm overflow-hidden rounded-xl bg-white shadow-md duration-200 hover:scale-105 hover:shadow-xl">
                <a
                  href={`/group/${d.Id}`}
                  key={i}
                  // style={{ width: '384px', height: '255px' }}
                >
                  <CardImage
                    imgSrcOrg={d.Properties.profileImage}
                    alt="plant"
                    className="h-auto w-full"
                    style={{ width: '384px', height: '255px' }}
                  />
                </a>

                <div className="p-5">
                  <a
                    href={`/group/${d.Id}`}
                    key={i}
                    // style={{ width: '384px', height: '255px' }}
                  >
                    <p className="text-medium mb-5 text-gray-700">{d.Title}</p>
                    <div className="grid grid-cols-2 gap-5">
                      <p>Teamcount: {d.TeamCount}</p>
                      <p>Role Name: {d.RoleName ? d.RoleName : 'Unknown'}</p>
                      <p>IsPending: {d.IsPending ? 'Yes' : 'No'}</p>
                      <p>IsManager: {d.IsManager ? 'Yes' : 'No'}</p>
                    </div>
                  </a>
                </div>
              </div>
            </div>
          ))}
        </div>
      )}
    </>
  );
};

export default AllGroup;
