...
const App = () => {
  ...
  const [groupIndividuals, setGroupIndividuals] = useState([]);
  ...
  useEffect(() => {
    if (!user && !runningAuth) {
      ...
      axios
        .get(
          `https://api-gagroupservice-dev.saams.xyz/api/v1/group/${pid}/collections`,
          config,
        )
        .then((res) => {
          console.log(res.data);
          setGroupIndividuals(res.data);
        })
        .catch((e) => {
          console.error(e);
        });
              ...
              <div className="w-full md:w-9/12 mx-2 h-64">
                ...
                <div className="bg-white p-3 shadow-sm rounded-sm">
                  <div className="grid grid-cols-4 gap-4">
                    {groupIndividuals.map((d, i) => (
                      <div>
                        <CircleImage
                          className="w-10 h-10 p-1 rounded-full ring-2 ring-gray-300 dark:ring-gray-500"
                          imgSrcOrg={d.ProfilePhoto}
                          alt="Bordered avatar"
                        />
                        <p>{d.Name}</p>
                      </div>
...
const CircleImage = ({ imgSrcOrg, ...props }) => {
